...
# -*- coding: utf-8 -*-
"""
kakao_auth.py — 카카오 로그인 OAuth 1회용 부트스트랩 CLI.

동작:
  1. .env의 KAKAO_REST_API_KEY를 읽는다(없으면 명확한 에러).
  2. 로컬 콜백 서버(http://localhost:8888/callback)를 띄운다.
  3. 브라우저로 인가 URL(scope=talk_message)을 열어 사용자 동의 → 인가 코드 수신.
  4. 인가 코드를 access/refresh 토큰으로 교환한다.
  5. .env에 KAKAO_REFRESH_TOKEN을 저장한다(기존 키 보존, 해당 키만 갱신).

공식 문서(developers.kakao.com) 검증:
  - 인가: GET https://kauth.kakao.com/oauth/authorize
          ?client_id=<REST_API_KEY>&redirect_uri=<URI>&response_type=code&scope=talk_message
  - 토큰: POST https://kauth.kakao.com/oauth/token
          grant_type=authorization_code, client_id, redirect_uri, code
  - refresh_token 유효기간 5184000초(60일).
...
CALLBACK_HOST = "127.0.0.1"   # localhost로 등록된 redirect_uri와 매핑됨
CALLBACK_PORT = 8888
...
def bootstrap(env_path: str = DEFAULT_ENV_PATH,
    ...
    if not rest_api_key:
        raise RuntimeError(
            f"{ENV_REST_API_KEY} 미설정 — .env에 REST API 키를 넣으세요. "
            f"(.env.example 참고, 콘솔 설정은 KAKAO_SETUP.md 확인)"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"콜백 포트 {CALLBACK_PORT} 바인딩 실패({e}). "
            f"다른 프로세스가 점유 중인지 확인하세요."
        ) from e

    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    logger.info("콜백 서버 시작: http://localhost:%d%s", CALLBACK_PORT, CALLBACK_PATH)
    ...
    print("아래 URL을 브라우저에서 열어 카카오 로그인/동의를 완료하세요:")
    print(auth_url)
    if not no_browser:
        try:
    ...
    if res.get("error"):
        raise RuntimeError(f"인가 실패: {res['error']} - {res.get('error_description')}")
    ...
    if not refresh_token:
        raise RuntimeError(f"토큰 응답에 refresh_token 없음: {parsed.get('error', 'unknown')}")

    update_env_var(ENV_REFRESH_TOKEN, refresh_token, env_path)
    logger.info("KAKAO_REFRESH_TOKEN 저장 완료 (%s)", mask_status(refresh_token))
    ...
    print(f"  refresh_token 유효기간: {parsed.get('refresh_token_expires_in')}초")
    print(f"  저장 위치: {env_path} ({ENV_REFRESH_TOKEN})")


def _main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="카카오 로그인 OAuth 1회용 부트스트랩")
    parser.add_argument("--env", default=DEFAULT_ENV_PATH, help=".env 경로 (기본 .env)")
    parser.add_argument("--redirect-uri", default=DEFAULT_REDIRECT_URI,
                        help=f"콘솔에 등록된 Redirect URI (기본 {DEFAULT_REDIRECT_URI})")
